...
if($_FILES['foto']['size'] > 0){
	$nombre = date("U");
	$directorio_guardar = '../img/premios/'.$nombre.'.jpg';
	$extension = pathinfo($_FILES['foto']['name'], PATHINFO_EXTENSION);
	if($extension == 'jpg'){
		$directorio_ruta = '../img/premios/'.$nombre.'.jpg';
	}
	elseif($extension == 'png'){
		$directorio_ruta = '../img/premios/'.$nombre.'.png';
	}
	...
	if(!@(move_uploaded_file($_FILES['foto']['tmp_name'],$directorio_ruta))){
		...
		$ancho_maximo = 151; // coloco aca el ancho maximo
		$alto_maximo = 151;  // coloco aca el alto maximo
		...
		if($im !== NULL){
			if(@imagesx($im) < $ancho_maximo){
				echo 'Error al intentar guardar la imagen.<br />';
				echo 'El ancho de la imagen debe superar los 151px.<br />';
				echo '<a href="premios.php">Volver a premios</a>';
				exit;
			}
...
if($_GET['accion'] == 1){
	$query = "INSERT INTO premios (titulo, texto, imagen) VALUES ('$titulo', '$texto', '$nombre_imagen')";
}
elseif($_GET['accion'] == 2){
	$id = $_POST['id'];
	$query = "UPDATE premios SET titulo='$titulo', texto='$texto', imagen='$nombre_imagen' WHERE id = $id";
}
