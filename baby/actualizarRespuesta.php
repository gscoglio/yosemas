...
$capturarIdRespuesta=mysql_query("SELECT * FROM respuestas WHERE id = $idrespuesta") or die("Problemas1 en el select:".mysql_error());
...
$capturarDatosPregunta=mysql_query("SELECT Pr.*, Ca.categoria FROM preguntas Pr
								INNER JOIN categorias Ca ON Ca.id = Pr.id_categoria 
								WHERE Pr.id = $idpregunta") or die("Problemas2 en el select:".mysql_error());
...
// SELECCIONO LOS USUARIOS GANADORES
$capturarGanadores = mysql_query("SELECT * FROM respuestas_usuarios WHERE id_respuesta = $idrespuesta AND id_pregunta = $idpregunta") 
or die("Problemas3 en el select:".mysql_error());
...
while($reg = mysql_fetch_array($capturarGanadores)) {
	...
	$capturarPuntos = mysql_query("SELECT * FROM puntaje WHERE id_usuario = $idusuario ORDER BY id_categoria") or die("Problemas4 en el select:".mysql_error());
...
$actuPregunta=mysql_query("UPDATE preguntas SET estado='".$estadoRespondida."' WHERE id = $idpregunta") or die("Problemas en el update:".mysql_error());

// Actualiza la calificacion de la respuesta correcta
$actuEstado=mysql_query("UPDATE respuestas SET correcta=1 WHERE id = $idrespuesta") or die("Problemas en el update:".mysql_error());
...
<DIV class="demo"  align="center">
    ...
    <table border="0" width="500">  
      ...
      <tr align="center">
           <td><h4>Para ver mas preguntas Finalizadas presiona en el boton Ver Preguntas Finalizadas. </h4></td>
      </tr>
      
      <tr align="center">
        <td> <a href="listarPreguntasFinalizadas.php">Ver Preguntas Finalizadas</a> </td>
      </tr>
